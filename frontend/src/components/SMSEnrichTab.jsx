                            ...
                            <div className="ml-14 mt-0.5 space-y-0.5">
                                {r.candidates.map((c, i) => (
                                    <div key={i} className="text-[10px] text-gray-500 flex items-center gap-2">
                                        ...
                                        {onPick && c.name && (
                                            <button
                                                onClick={() => onPick(r.transaction_id, c.name, c.raw_sms, c.timestamp)}
...
const SMSEnrichTab = ({ onApplied }) => {
    ...
    const pickCandidate = async (transactionId, name, rawSms, smsTimestamp) => {
        setPicking(transactionId);
        setError(null);
        try {
            await api.post(`${API_URL}/api/sms/enrich/apply`, {
                items: [{ transaction_id: transactionId, new_merchant: name, raw_sms: rawSms, sms_timestamp: smsTimestamp }],
            });
            await loadReport();
            loadBatches();
    ...
    const apply = async () => {
        const items = proposals
            ...
            .map(p => ({ transaction_id: p.transaction_id, new_merchant: p.new_merchant, raw_sms: p.raw_sms, sms_timestamp: p.sms_timestamp }));
