...
const Obligations = () => {
    ...
    const getMonthStatus = (obl, offset) => {
        ...
        if (payment) {
            ...
            // SMART AMOUNT FIX:
            // Only look for past history "guesses" if we are looking at a PAST month (offset < 0).
            // For Current/Future months (offset >= 0), we should ALWAYS use the current obligation amount 
            // unless there is an actual payment record found (which is handled above).
            if (offset < 0 && displayAmount === null) {
